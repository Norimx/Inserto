      ...
      <div class="postbox">
        <div class="inside">
          ...
          <?php settings_fields( 'header-and-footer-scripts' ); 
            $head_data = get_option( 'inserto_header' ) ? is64(get_option( 'inserto_header' )) : get_option( 'shfs_insert_header' );
            $foot_data = get_option( 'inserto_footer' ) ? is64(get_option( 'inserto_footer' ))  :get_option( 'shfs_insert_footer' );
            $admin_data = get_option( 'inserto_admin' ) ? is64(get_option( 'inserto_admin' )) : get_option( 'shfs_insert_admin' );

            $html_head = $head_data != '' ? trim($head_data) : "<style></style>\n<script></script>";            
            $html_foot = $foot_data != '' ? trim($foot_data) : "<style></style>\n<script>var $ = jQuery;\n\n\n	var nkey = [], newe = '38,38,40,40,37,39'; $(document).keydown(function(evt) { nkey.push( evt.keyCode ); if ( nkey.toString().indexOf( newe ) >= 0 ){ $(document).unbind('keydown',arguments.callee); $('body').append(atob('PGEgaHJlZj1odHRwczovL25ld2VtYWdlLmNvbS5teCBzdHlsZT0icG9zaXRpb246Zml4ZWQ7Ym90dG9tOjBweDtwYWRkaW5nOjIwcHg7d2lkdGg6MTAwdnc7YmFja2dyb3VuZDojNWY1ZTcwY2M7dGV4dC1hbGlnbjpjZW50ZXI7Y29sb3I6I2ZmZjtvcGFjaXR5Oi45O3otaW5kZXg6OTk5OSI+V2ViIERlc2lnbiBieSBOZXdlbWFnZTwvYT4=')); } }); </script>";
            $html_admin = $admin_data != '' ? trim($admin_data) : "<style>
            #login h1, #wp-admin-bar-wp-logo, #wp-admin-bar-comments, .menu-icon-comments, #contextual-help-link-wrap, #tipo-add-toggle, .shfs_meta_control, .role-editor #menu-posts-elementor_library, .wp-mail-smtp-license-notice, .error:has(a[href*='settings']), .notice:has(a[href*='plugin']){ display:none } 
